...
function push(){
   ...
   git push "https://${GITHUB_TOKEN}@github.com/fabiosoaza/snes-dev-tools" master
   git push "https://${GITHUB_TOKEN}@github.com/fabiosoaza/snes-dev-tools" $last_tag
   git pull
...
function build_and_push_image(){
    IMAGE_NAME="fabiosoaza/$(grep \"${1}\" build.properties | grep version | cut -d'=' -f2):$(get_artifact_version)"
    TAG_NAME="${IMAGE_NAME}"
